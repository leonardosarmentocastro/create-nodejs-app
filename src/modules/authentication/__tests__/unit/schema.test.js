...
test('(preSaveMiddleware) must hash the password if still not hashed', async t => {
  ...
  t.truthy(await authenticationEncrypter.verify(schema.password, unhashedPassword)); // while still matching the unhashed value
});

test('(preSaveMiddleware) must not hash the password again if already hashed', async t => {
  const hashedPassword = await authenticationEncrypter.hash('hashed password');
  ...
  t.truthy(await authenticationEncrypter.verify(schema.password, 'hashed password')); // and still matches the old value
});
